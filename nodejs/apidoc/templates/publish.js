...
function addAttribs(f) {
    ...
    if (attribs.length) {
        var span = '<span class="type-signature ';
        
        if ( attribs.indexOf('virtual') > -1 ) {
            attribs[ attribs.indexOf('virtual') ] = 'abstract';
            span += 'abstract';
        }
        
        // attribs[ attribs.indexOf('virtual') ] = ( attribs.indexOf('virtual') > -1 ) ? "abstract" : "virtual"
        f.attribs = span + (attribs[0] === 'static' ? 'static' : '') + '">' + htmlsafe(attribs.length ? attribs.join(',') : '') + '</span>';
    }    
}
